...
const Hero: React.FC<HeroProps> = ({ items, onSelectFavorite }) => {
    useEffect(() => {
        ...
        const sectionHero = document.querySelector(
            '.section-hero__wrapper .hero-parallax'
        ) as HTMLElement | null;
        ...
        const scrollBtn = document.querySelector(
            '.section-hero__wrapper .scroll-btn'
        ) as HTMLElement | null;
        const aboutSwiper = document.querySelector(
            '.section-about__swiper'
        ) as HTMLElement | null;

        const buildModuleHeader = (sectionHero: HTMLElement) => {};

        const effectsModuleHeader = (
            sectionHero: HTMLElement | null,
            scrollTopp: HTMLElement
        ) => {
            if (sectionHero) {
                const homeSHeight = sectionHero.offsetHeight;
                const topScroll = document.documentElement.scrollTop;
                if (
                    sectionHero.classList.contains('hero-parallax') &&
                    topScroll <= homeSHeight
                ) {
                    sectionHero.style.transform = `translateY(${
                        topScroll * 0.5
                    }px)`;
                    if (navigation) {
                        navigation.style.transform = `translateY(${
                            topScroll * 0.575
                        }px)`;
                    }
                    if (scrollBtn) {
                        scrollBtn.style.transform = `translateY(${
                            topScroll * -0.175
                        }px)`;
                    }
                    if (aboutSwiper) {
                        aboutSwiper.style.transform = `translateY(${
                            topScroll * -0.033
                        }px)`;
                    }
                }
                if (
                    ...
                    sectionHero.style.opacity = `${
                        1 - topScroll / sectionHero.offsetHeight
                    }`;
                }
            }
        };
